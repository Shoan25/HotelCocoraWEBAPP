...
var index;
for(index in roomsToShowX){

var roomID = roomsToShowX[index];

var index2;


for(index2 in rooms){
    var room = rooms[index2]
    if(roomID==room._id){
        ...
        <div class="content">
           ...
           <a id="btnBooking" name="${room._id}" class="btn">book now</a>
        </div>
     </div>
...
const buttons = document.querySelector(".swiper-wrapper");
...
buttons.onclick = (e) => {
  ...
  var roomToShowFinal=[];
  localStorage.setItem("RoomToShowFinal", JSON.stringify(roomToShowFinal));

  roomToShowFinal.push(value);
  localStorage.setItem("RoomToShowFinal", JSON.stringify(roomToShowFinal));

  window.location.href = "finalReserva.html";
}
